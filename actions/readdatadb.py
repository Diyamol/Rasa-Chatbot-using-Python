...
class Botapp_fundingprogramtbl(Base):
    ...
    def __repr__(self):
        return "<Botapp_fundingprogramtbl(id= '%d', program_name= '%s', about_program='%s', who_can_apply = '%s', deadline='%s', how_to_apply= '%s', link = '%s', created_at='%s', updated_at='%s', language = '%s', program_id='%s', keywords = '%s', programTypeId = '%s',statusId = '%s',reserveId = '%s',eligibility = '%s', isApproved = '%d')>" % (
                                self.id, self.program_name, self.about_program, self.who_can_apply, self.deadline, self.how_to_apply, self.link, self.created_at, self.updated_at, self.language, self.program_id, self.keywords,self.programTypeId,self.statusId,self.reserveId,self.eligibility,self.isApproved)
...
def fetch_about_program(program_name):
    ...
    bottab = session.query(Botapp_fundingprogramtbl).filter(Botapp_fundingprogramtbl.program_name.like('%'+program_name+'%')).all()
    # for val in bottab
    # if(bottab.isApproved == 2):
    #     return bottab.about_program
    return bottab

def fetch_details_program(program_name):
    #program_name = 'Education Partnerships Program'
    engine = create_engine("mysql+pymysql://finuser@devtrustd:Stablehacks123@devtrustd.mysql.database.azure.com/finance")
    metadata = sqlalchemy.MetaData()
    #Botapp_fundingprogramtbl = Table('botapp_fundingprogramtbl', metadata, autoload=True, autoload_with=engine)
...
def fetch_who_can_apply(program_name):
    ...
    bottab = session.query(Botapp_fundingprogramtbl).filter(Botapp_fundingprogramtbl.program_name.like('%'+program_name+'%')).first()
...
def fetch_bussinessSupport(program_name):
    #program_name = 'Education Partnerships Program'
    engine = create_engine("mysql+pymysql://finuser@devtrustd:Stablehacks123@devtrustd.mysql.database.azure.com/finance")
    metadata = sqlalchemy.MetaData()
    Session = sessionmaker(bind=engine)
